...
export default class DashAnalyticsComponent {
  ...
  convertTimestampToDate(timestamp: { seconds: number, nanoseconds: number }): string {
    const date = new Date(timestamp.seconds * 1000);  // Convertimos los segundos en milisegundos
    return date.toLocaleString();  // Puedes formatear la fecha según sea necesario
  }
